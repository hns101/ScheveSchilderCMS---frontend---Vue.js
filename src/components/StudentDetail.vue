...
const fetchStudentDetail = async () => {
  try {
    ...
    const response = await axios.get<any>(`/api/students/${studentId}`);

    // Handle both old format (direct data) and new format (ApiResponse)
    if (response.data && response.data.success !== undefined) {
      // New API format with ApiResponse wrapper
      if (response.data.success) {
        student.value = response.data.data;
      } else {
        error.value = response.data.message || "Fout bij het laden van studentdetails";
      }
    } else {
...
const deleteStudent = async (id: string) => {
  if (confirm('Weet u zeker dat u deze student en alle bijbehorende facturen wilt verwijderen?')) {
    try {
      ...
      let errorMessage = "Fout bij het verwijderen van student";
...
const formatDate = (dateValue: string | Date | null | undefined) => {
  if (!dateValue) return 'Niet opgegeven';
  try {
