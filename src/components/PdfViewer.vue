...
### **Step 2: Update `router/index.ts` (New Route for PDF Viewer)**

We need to add a new dynamic route for the PDF viewer. This route will take two parameters: `id` (the student or invoice ID) and `type` (to differentiate between student documents and invoices).
...
### **Step 3: Update `StudentDetail.vue` (Link to PDF Viewer)**

Instead of a direct link, the "Bekijk document" link will now navigate to the new `PdfViewer` route.
  ...
  <div class="page-container">
    ...
    <div v-if="student" class="student-details-card">
      ...
      <div class="detail-group" v-if="student.registrationDocumentPath">
        ...
        <a href="#" @click.prevent="viewPdf('student', student.id!)" class="detail-link">Bekijk document</a>
      </div>
      ...
      <div v-if="student.invoices && student.invoices.length" class="invoice-list">
        <div v-for="invoice in student.invoices" :key="invoice.id" class="invoice-item">
          ...
          <a href="#" @click.prevent="viewPdf('invoice', invoice.id!)" class="detail-link">Bekijk PDF</a>
        </div>
      </div>
...
<script setup lang="ts">
  ...
  const viewPdf = (type: string, id: string) => {
...
I have carefully reviewed your request and the code you provided, and I will proceed with implementing the file upload functionality into the `StudentForm.vue` component. I will ensure all new code adheres to your existing styling and design.

The primary issues in your `StudentForm.vue` are:
1.  **Missing File Input:** The form needs an input element of `type="file"`.
2.  **Missing File Handling Logic:** The component needs to manage the selected file in its state.
3.  **Incorrect Submission Logic:** The `submitForm` method must handle `multipart/form-data` for file uploads and a regular `PUT` for updates without a file.

I have updated the `StudentForm.vue` component to correctly handle these points. I have been extremely careful to preserve all of your CSS.

---

### **1. Updated `StudentForm.vue` (Handles File Upload)**

I've updated the `StudentForm.vue` component to include a file input field for the registration document. It will handle the following logic:
* **File Upload:** The form now sends a `multipart/form-data` request when a new student is created with a file.
* **File Deletion (for edits):** When editing, if a document already exists, a "Verwijder" (Delete) button will appear, which sends a `DELETE` request to your backend.
* **File Update (for edits):** When a new file is selected during an edit, the form will send a `POST` request to the file upload endpoint to replace the existing document.


[Immersive content redacted for brevit